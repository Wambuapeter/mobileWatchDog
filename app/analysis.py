...
def analyze_apk(task, scan_id):
    ...
    try:
        ...
        scan.status = 'watchDog is Getting info of apk'
        scan.progress = 5
        scan.save()
        task.update_state(state = 'STARTED',
                meta = {'current': scan.progress, 'total': 100, 'status': scan.status})
        ...
        scan.status = 'WatchDog is Getting info of certificates'
        scan.progress = 10
        scan.save()
        #scanning -> analysis.py
        task.update_state(state = 'STARTED',
                meta = {'current': scan.progress, 'total': 100, 'status': scan.status})
        logger.debug(scan.status)
        get_info_certificate(a, scan)
        if (settings.VIRUSTOTAL_ENABLED):
            scan.status = 'watchDog is Getting info of VT'
            scan.progress = 15
            scan.save()
            task.update_state(state = 'STARTED',
                meta = {'current': scan.progress, 'total': 100, 'status': scan.status})
        ...
        scan.status = 'watchDog is decompiling'
        scan.progress = 20
        scan.save()
        task.update_state(state = 'STARTED',
                meta = {'current': scan.progress, 'total': 100, 'status': scan.status})
        ...
        scan.status = 'watchDog is Finding vulnerabilities'
        scan.progress = 40
        task.update_state(state = 'STARTED',
                meta = {'current': scan.progress, 'total': 100, 'status': scan.status})
        scan.save()
...
def get_info_certificate(a, scan):
    ...
    if a.is_signed():
        ...
        for cert in a.get_certificates():
            # Each cert is a asn1crypt.x509.Certificate object
